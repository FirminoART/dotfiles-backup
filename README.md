...
- **Hardware captured:** Intel-only iGPU (`Intel Whiskey Lake-U UHD 610`,
  ...
  **not** installed). Laptop panel `eDP-1 1366x768@60.02`, secondary
  `HDMI-A-1` (see `monitors.conf`). Keyboard layout **br** (ABNT2).
...
- [ ] Multi-monitor: run **`nwg-displays`** — it rewrites `monitors.conf` +
      `workspaces.conf` (+ `.lua` twins). Current files target
      `eDP-1 1366x768@60.02` + `HDMI-A-1` (§6).
...
- `hypr/workspaces.conf` + `workspaces.lua` — ws1→eDP-1, ws2→HDMI-A-1.
...
- `hypr/hyprpaper.conf` — wallpaper `~/Pictures/Wallpapers/angel.png` on
  both monitors, `splash=off, ipc=off`.
...
- **Wallpaper path is hard-coded** in `hyprpaper.conf`, `hyprlock.conf`
  (`~/Pictures/Wallpapers/angel.png`). `install.sh` guarantees that path.
